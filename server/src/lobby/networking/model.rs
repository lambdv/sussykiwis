...
impl ServerEvent {
    pub fn message_for(self, id: Uuid) -> Option<ServerResponse> {
        match self {
            ServerEvent::Broadcast(message) => Some(message),
            ServerEvent::Direct { to, message } if to == id => Some(message),
            _ => None,
        }
    }
}
...
pub struct WorldSnapshot {
    ...
    pub map_half_extent_x: f32,
    pub map_half_extent_z: f32,
